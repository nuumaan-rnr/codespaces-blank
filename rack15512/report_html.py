...
def design_validation_report(model: RackModel, cases: List[CaseResult],
    ...
    if uset:
        grp_ok = all(r["status"] == "PASS" for r in uset)
        a(f"<h3>6.{n} UPRIGHT BUCKLING BY MEMBER-SET &mdash; "
          "EN 15512 9.7.2</h3>")
        n += 1
        a("<p class='basis'>Each continuous upright is grouped into storey "
          "segments (base&rarr;L1, L1&rarr;L2, …); Lcr,DA is the down-aisle and "
          "Lcr,CA the cross-aisle buckling length. The governing element "
          "(with its N, My, Mz) represents the set.</p>")
        cls = "pass" if grp_ok else "fail"
        a(f"<p class='groupres {cls}'>{'PASS' if grp_ok else 'FAIL'} — "
          f"worst utilisation {uset[0]['util']:.3f} on "
          f"{_esc(uset[0]['set'])} ({_esc(uset[0]['case'])}).</p>")
        a("<table><thead><tr><th>Set</th><th>Lcr,DA (mm)</th>"
          "<th>Lcr,CA (mm)</th><th>N (kN)</th><th>My (kNm)</th>"
          "<th>Mz (kNm)</th><th>Gov. element</th><th>Utilisation</th>"
          "<th>Case</th><th>Status</th></tr></thead><tbody>")
        for r in uset:
            scls = "okrow" if r["status"] == "PASS" else "failrow"
            a(f"<tr class='{scls}'><td>{_esc(r['set'])}</td>"
              f"<td>{r['Lcr_DA_mm']}</td><td>{r['Lcr_CA_mm']}</td>"
              f"<td>{r['N_kN']:.2f}</td><td>{r['My_kNm']:.2f}</td>"
              f"<td>{r['Mz_kNm']:.2f}</td><td>member {r['member']}</td>"
              f"<td>{r['util']:.3f}</td><td>{_esc(r['case'])}</td>"
              f"<td>{r['status']}</td></tr>")
        a("</tbody></table>")

    # suggested upright splice positions (advisory)
    from .checks.en15512 import suggest_splice_positions
    sug = suggest_splice_positions(model, cases)
    if sug["n_needed"] > 0 and sug["candidates"]:
        a(f"<h3>6.{n} SUGGESTED UPRIGHT SPLICE POSITIONS</h3>")
        n += 1
        a(f"<p class='basis'>Frame {sug['H']:.0f} mm needs &ge; "
          f"{sug['n_needed']} splice(s) for a {sug['max_length']:.0f} mm max "
          "manufacturable length. Place just above a beam level near low "
          "down-aisle moment (EN 1993-1-8 §6.2.7.1); one per inter-beam zone, "
          "not in the lowest panel. A splice away from a restraint must be "
          "checked for the second-order strut moment and develop &ge; 25% of "
          "the upright compressive force.</p>")
        a("<table><thead><tr><th>Elevation (mm)</th><th>Mz (kNm)</th>"
          "<th>Suggested</th><th>Note</th></tr></thead><tbody>")
        for c in sug["candidates"]:
            cls = "okrow" if c["recommended"] else ""
            a(f"<tr class='{cls}'><td>{c['z']:.0f}</td>"
              f"<td>{c['Mz_kNm']:.2f}</td>"
              f"<td>{'&check;' if c['recommended'] else ''}</td>"
