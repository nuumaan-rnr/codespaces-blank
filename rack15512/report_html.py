...
def design_validation_report(model: RackModel, cases: List[CaseResult],
    ...
    if uset:
        grp_ok = all(r["status"] == "ok" for r in uset)
        a(f"<h3>6.{n} UPRIGHT BUCKLING BY MEMBER-SET &mdash; "
          "EN 15512 9.7.2</h3>")
        n += 1
        a("<p class='basis'>Each continuous upright is grouped into storey "
          "segments (base&rarr;L1, L1&rarr;L2, …); the set length is the "
          "down-aisle buckling length Lcr,DA. The governing element "
        ...
        a("<table><thead><tr><th>Set</th><th>Lcr,DA (mm)</th>"
          "<th>Gov. element</th><th>Utilisation</th><th>Case</th>"
          "<th>Status</th></tr></thead><tbody>")
        for r in uset:
            scls = "okrow" if r["status"] == "ok" else "failrow"
            a(f"<tr class='{scls}'><td>{_esc(r['set'])}</td>"
              f"<td>{r['Lcr_DA_mm']}</td><td>member {r['member']}</td>"
