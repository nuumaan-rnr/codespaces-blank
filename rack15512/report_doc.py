...
def build_report_blocks(model, cases, checks, meta=None) -> List[tuple]:
    ...
    if uset:
        ...
        b.append(("h3", f"6.{n}  UPRIGHT BUCKLING BY MEMBER-SET - EN 15512 9.7.2"))
        n += 1
        b.append(("note", "Each continuous upright is grouped into storey "
                          "segments (base->L1, L1->L2, ...); Lcr,DA is the "
                          "down-aisle and Lcr,CA the cross-aisle buckling length. "
                          "The governing element (with N, My, Mz) represents the "
                          "set."))
        b.append(("result",
                  f"{'PASS => o.k.' if grp_ok else 'FAIL => NOT o.k.'}  "
                  f"(worst utilisation {uset[0]['util']:.3f} on "
                  f"{uset[0]['set']}, {uset[0]['case']})", grp_ok))
        trows = [["Set", "Lcr,DA (mm)", "Lcr,CA (mm)", "N (kN)", "My (kNm)",
                  "Mz (kNm)", "Gov. element", "Util.", "Case", "Status"]]
        for r in uset:
            trows.append([r["set"], str(r["Lcr_DA_mm"]), str(r["Lcr_CA_mm"]),
                          f"{r['N_kN']:.2f}", f"{r['My_kNm']:.2f}",
                          f"{r['Mz_kNm']:.2f}", f"member {r['member']}",
                          f"{r['util']:.3f}", r["case"], r["status"]])
        b.append(("table", trows))
    ...
    if sug["n_needed"] > 0 and sug["candidates"]:
        b.append(("h3", f"6.{n}  SUGGESTED UPRIGHT SPLICE POSITIONS"))
        n += 1
        b.append(("note", f"Frame {sug['H']:.0f} mm needs >= {sug['n_needed']} "
                          f"splice(s) for a {sug['max_length']:.0f} mm max "
                          "manufacturable length. Place just above a beam level "
                          "near low down-aisle moment (EN 1993-1-8 6.2.7.1); "
                          "one splice per inter-beam zone, not in the lowest "
                          "panel. Splices away from a restraint must be checked "
                          "for the second-order strut moment and develop >= 25% "
                          "of the upright compressive force."))
        srows = [["Elevation (mm)", "Mz (kNm)", "Suggested", "Note"]]
        for c in sug["candidates"]:
            srows.append([f"{c['z']:.0f}", f"{c['Mz_kNm']:.2f}",
                          "yes" if c["recommended"] else "", c["note"]])
        b.append(("table", srows))
