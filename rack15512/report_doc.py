...
def build_report_blocks(model, cases, checks, meta=None) -> List[tuple]:
    ...
    if uset:
        ...
        b.append(("note", "Each continuous upright is grouped into storey "
                          "segments (base->L1, L1->L2, ...); the set length is "
                          "the down-aisle buckling length Lcr,DA. The governing "
                          "element represents the set."))
        b.append(("result",
                  f"{'PASS => o.k.' if grp_ok else 'FAIL => NOT o.k.'}  "
                  f"(worst utilisation {uset[0]['util']:.3f} on "
                  f"{uset[0]['set']}, {uset[0]['case']})", grp_ok))
        trows = [["Set", "Lcr,DA (mm)", "Gov. element", "Util.", "Case",
                  "Status"]]
        for r in uset:
            trows.append([r["set"], str(r["Lcr_DA_mm"]),
                          f"member {r['member']}", f"{r['util']:.3f}",
                          r["case"], r["status"].upper()])
        b.append(("table", trows))
