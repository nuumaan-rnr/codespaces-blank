...
CHECK_TITLES = {
    "cross_section": "Cross-section resistance (EN 15512 9.7.2)",
    "buckling": "Flexural buckling of uprights (EN 15512 9.7 / EC3 6.3)",
    "connector": "Beam-end connector moment (test-based M_Rd)",
    "deflection": "Pallet beam deflection (SLS)",
    "sway": "Down-aisle sway (SLS)",
}
...
def to_markdown(cfg: RackConfig, model: RackModel, results: AnalysisResults,
    ...
    lines = [
        ...
        f"bay width {geo.bay_width:.2f} m, total height {geo.total_height:.2f} m",
        ...
        f"- Sway imperfection phi = {cfg.sway_imperfection():.5f} rad "
    ...
    lines += [
        "---",
        "*Stress, buckling, connector checks at ULS from second-order analysis "
        "with sway imperfections; deflection and sway at SLS. Verify imperfection "
        "values, partial factors and test-based section/connection properties "
        "against the EN 15512 edition applicable to the project.*",
    ]
    return "\n".join(lines)
...
def to_json(cfg: RackConfig, model: RackModel, results: AnalysisResults,
    ...
    payload = {
        ...
        "sway_imperfection": cfg.sway_imperfection(),
        "combinations": {
            cid: {
